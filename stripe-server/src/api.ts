...
/**
 * By default express handles the incoming body as string
 * which means we have to decode it and parse in it to JSON
 * on every request. We can avoid this by setting up middleware
 * which changes the behaviour of express. This is done by using
 * .use method on our express app, for this we'll use the middleware
 * provided by express
 */
